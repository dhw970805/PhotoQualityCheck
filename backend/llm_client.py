...
from config import QWEN_API_KEY, QWEN_API_URL, QWEN_MODEL, QWEN_TIMEOUT, LLM_IMAGE_MAX_WIDTH, BATCH_SIZE
...
def analyze_batch_with_llm(image_paths):
    ...
    try:
        ...
        t_request = time.time() - t1
        t_encode = t1 - t0
        logger.info(f"LLM batch response: {response.status_code}, "
                     f"total={t_encode + t_request:.1f}s (encode={t_encode:.2f}s, api={t_request:.2f}s)")
        ...
        if not isinstance(results_list, list):
            logger.error(f"Expected JSON array but got {type(results).__name__}")
            return [None] * len(image_paths)
